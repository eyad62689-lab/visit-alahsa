...
- **الأسرار لا تنتقل تلقائياً إلى المعاينات.** إن كانت متغيرات البيئة محصورة بسياق `production`، فمعاينات النشر ستُبنى بلا `GOOGLE_PLACES_API_KEY` (تقييمات ناقصة — لا يُفشل البناء) وبلا `IP_HASH_SALT` (الحماية تخزّن بلا بصمة IP). راجع نطاق كل متغير في *Environment variables* ← عمود *Scopes*. **تحديث ب.٢ (PR #10، 2026-09-04):** صار `fetch-places.mjs` يتخطّى نداء Places كلياً حين `CONTEXT` موجود وليس `production` — فالمعاينات بلا تقييمات **بالتصميم** لا بغياب المفتاح، وحصرُ المفتاح في Production بعد الدمج يمنع إنفاقاً لا فائدة منه ولا يغيّر مخرج المعاينة.
