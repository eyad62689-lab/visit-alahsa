...
//
// النداء محصور في بناء الإنتاج (البند ب.٢، 2026-09-04): Netlify تحقن `CONTEXT`
// (production | deploy-preview | branch-deploy | dev)، وفي غير production يُتخطّى
// النداء بتحذير ولا يُفشل البناء — فالمعاينات تُبنى بلا تقييمات كما لو غاب المفتاح.
// علّته: كل معاينة كانت تشتري 40 استدعاء تفاصيل (20 مكاناً × لغتين) لبيانات لا
// تُخزَّن أصلاً بشرط قوقل، وحصر المفتاح في Production على Netlify يسبق هذا الشرط
// وإلا صار غياب المفتاح في المعاينات هو الطريق الوحيد. **بلا `CONTEXT`** (تشغيل
// محلي) يعمل السكربت كما كان: هو الطريق الموثَّق لتوليد place-ids.json محلياً.
//
...
// سياق النشر الذي تحقنه Netlify؛ غيابه = بناء محلي لا معاينة.
...
async function main() {
  if (CONTEXT && CONTEXT !== 'production') {
    // يُفحص قبل المفتاح عمداً: وجود المفتاح في المعاينة لا يبرّر إنفاقه، وغيابه
    // بعد حصره في Production لا يستحق تحذير «أضف المفتاح» المضلِّل.
    console.warn(`fetch-places: سياق النشر «${CONTEXT}» ليس production — تُخطّي نداء Places API عمداً.`);
    console.warn('صفحة المطاعم والمقاهي تُبنى بلا تقييمات ولا أوقات عمل في هذا السياق (البيانات لا تُخزَّن بشرط قوقل).');
    return;
  }
